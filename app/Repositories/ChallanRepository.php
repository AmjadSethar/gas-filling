...
class InvoiceRepository extends BaseRepository
{
    ...
    public function getSyncList($invoice = []): array
    {
        ...
        $data['discount_type'] = Challan::DISCOUNT_TYPE;
        $invoiceStatusArr = Challan::STATUS_ARR;
        unset($invoiceStatusArr[Challan::STATUS_ALL]);
        $invoiceRecurringArr = Challan::RECURRING_ARR;
    ...
    public function saveInvoice(array $input): Challan
    {
        try {
            DB::beginTransaction();
            ...
            $invoiceExist = Challan::where('invoice_id', $input['invoice_id'])->exists();
            ...
            if ($invoiceExist) {
                throw new UnprocessableEntityHttpException('Challan id already exist');
            }

            if (isset($input['product_id']) && isset($input['client_id'])) {
                $clientId = Client::whereUserId($input['client_id'])->first()->id;
                Product::whereIn('id', $input['product_id'])->update(['client_id' => $clientId]);
            }
            /** @var Challan $invoice */
            ...
            if ($invoice->status != Challan::DRAFT) {
    ...
    /**
     * @return Challan|Builder|Builder[]|Collection|Model
     */
    ...
    public function saveNotification(array $input, $invoice = null): void
    {
        ...
        if ($input['status'] != Challan::DRAFT) {
            addNotification([
                Notification::NOTIFICATION_TYPE['Challan Created'],
                $userId,
                $title,
            ]);
        }
    ...
    public function updateNotification($invoice, $input, array $changes = [])
    {
        ...
        if ($input['status'] != Challan::DRAFT) {
            ...
            addNotification([
                Notification::NOTIFICATION_TYPE['Challan Updated'],
                $userId,
                $title,
            ]);
        }
