...
public function columns(): array
{
    return [
        ...
        Column::make(__('messages.product.wall_type'), 'wall_type')
            ->sortable()
            ->searchable(),
        ...
        Column::make(__('messages.product.address'), 'client.address')
            ->sortable()
            ->searchable()
            ->format(function ($value, $row, Column $column) {
                return $row->client->address ?? 'At Shop';
            }),
    ...
    {
        return Product::with(['category', 'client'])->select('products.*');
    }
